...
class AdaptadorBoxes(val lista_mensajes: List<Mensaje>, actividad: AppCompatActivity, contexto: Context): RecyclerView.Adapter<AdaptadorBoxes.BocadilloViewHolder>(){
    ...
    override fun onBindViewHolder(holder: BocadilloViewHolder, position: Int) {
        ...
        if(item_actual.emisor_id == cliente.id){
            ...
            holder.items_emisor.avatar.setOnClickListener {
                ...
                if(cliente.admin!! && cliente.id != item_actual.emisor_id){

                    verUsuario("id", item_actual.emisor_id){
                        if(it != null){
                            if(it.disponible){
                                val intencion = Intent(contexto, EditarUsuario::class.java)
                                intencion.putExtra("revisar", it)
                                startActivity(contexto, intencion, null)
                            }else{
                                Toast.makeText(contexto, "Usuario no disponible", Toast.LENGTH_SHORT).show()
                            }
                        }else{
